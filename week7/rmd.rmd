...
### 1. **Train several models using `caret` on the `mnist_27` dataset**
...
### 2. **Create a matrix of predictions for the test set**
...
### 3. **Compute accuracy for each model**
...
### 4. **Build an ensemble prediction by majority vote and compute its accuracy**
...
### 5. **Compare individual models to the ensemble**
...
### 6. **Use cross-validation accuracy estimates to filter models**
...
### 8. **Advanced: Visualize model similarities using a heatmap**
...
### 9. **Advanced: Use conditional probabilities to create an ensemble**
...
### 10. **Fit a model to the entire MNIST dataset**

We follow the same process as above, but apply it to the full MNIST dataset instead of `mnist_27`.
